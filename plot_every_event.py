...
def import_catalog(file):
    ...
    df['arrival_datetime'] = df.arrival_time.apply(mdates.num2date)
    return df
...
def plot_event(event_id, day_number, df, script=False):
    ...
    if script == True:
        fig.savefig(f'eventfigs/{day_number}_{event_id}.png', dpi=300, bbox_inches='tight')
        del fig, ax, e
...
if __name__=='__main__':
    ...
    for id in tqdm(df141['id'].values):
        plot_event(event_id=id, day_number=141, df=df141, script=True)
